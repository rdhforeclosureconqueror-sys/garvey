(function(root, factory) {
  ...
  const QUESTION_TYPES=['multiple_choice', 'short_response', 'rounding', 'fraction_response', 'multiplication_equation', 'division_equation', 'word_problem', 'operation_sort', 'fraction_entry', 'word_building', 'syllable_break', 'matching', 'letter_identification', 'sound_match', 'picture_choice', 'visual_objects', 'number_sequence', 'number_line', 'comparison', 'comparison_cards', 'base_ten_blocks', 'place_value_chart', 'expanded_form', 'addition_model', 'subtraction_model', 'number_bond', 'ten_frame', 'shape_identification', 'measurement_comparison', 'pattern_completion', 'sorting_visual', 'analog_clock', 'digital_time', 'time_matching', 'word_problem_model', 'bar_model', 'equation_builder', 'ruler', 'coin_model', 'money_counting', 'data_interpretation', 'graph_reading', 'visual_selection', 'array_counting', 'sound_position', 'same_different_sound', 'word_builder', 'sound_box_blend', 'decode_word', 'word_family_sort', 'rhyme_match', 'sight_word_match', 'sentence_cloze', 'sentence_completion', 'phrase_reading', 'sentence_picture_match', 'punctuation_reading', 'text_evidence_choice', 'text_evidence', 'wh_question', 'event_ordering', 'sequencing', 'picture_order', 'sequence_select', 'sentence_builder', 'capitalization_fix', 'spacing_fix', 'punctuation_choice', 'constructed_sentence', 'picture_prompt_response', 'detail_picker', 'detail_match', 'topic_identification', 'main_idea_choice', 'rubric_scored_writing', 'writing_response', 'vocabulary_match', 'category_sort', 'elapsed_time', 'measurement', 'area_response', 'perimeter_response', 'sorting'];
  const VISUAL_MODELS=['visual_objects', 'letter_card', 'sound_match', 'picture_choice', 'phonics_tiles', 'sound_boxes', 'syllable_break', 'word_card', 'sentence_card', 'number_sequence', 'number_line', 'number_line_0_120', 'comparison', 'comparison_cards', 'base_ten_blocks', 'place_value_chart', 'expanded_form', 'addition_model', 'subtraction_model', 'number_bond', 'ten_frame', 'shape_identification', 'measurement_comparison', 'pattern_completion', 'sorting_visual', 'analog_clock', 'digital_time', 'time_matching', 'word_problem_model', 'bar_model', 'equation_builder', 'ruler', 'coin_model', 'money_counting', 'picture_graph', 'bar_graph', 'line_plot', 'data_table', 'partition_shapes', 'array_model', 'equal_groups', 'repeated_addition', 'multiplication_model', 'division_model', 'fact_family_model', 'multiplication_chart', 'skip_counting', 'operation_sort', 'rounding_model', 'fraction_bar', 'fraction_circle', 'word_sound_map', 'word_builder', 'word_family_sort', 'rhyme_match', 'sentence_highlight', 'phrase_builder', 'sentence_builder', 'short_passage', 'evidence_highlight', 'story_map', 'main_idea_web', 'detail_cards', 'picture_story', 'question_card', 'story_sequence', 'picture_order', 'event_cards', 'writing_checklist', 'opinion_reason_chart', 'paragraph_builder', 'fact_cards', 'topic_detail_chart', 'punctuation_marker', 'picture_prompt', 'detail_picker', 'word_parts', 'morpheme_tiles', 'context_sentence', 'vocabulary_match', 'category_sort', 'elapsed_time_timeline', 'area_model', 'grid_model', 'perimeter_path', 'rectangle_model', 'attribute_sort'];
  ...
  const AUDIO_TYPES=['letter_sound','word','sentence','instruction','phoneme'];
  ...
  function validateAudioSupport(item,zone,errors,warnings){if(!item||typeof item!=='object')return; const audio=item.audio; if(audio!==undefined){if(!audio||typeof audio!=='object'||Array.isArray(audio)){errors.push(`${zone}.audio must be an object`);} else {if(!has(audio,'text')){errors.push(`${zone}.audio.text is required when audio is provided`);} if(audio.type&&!AUDIO_TYPES.includes(audio.type)){errors.push(`${zone}.audio.type must be one of ${AUDIO_TYPES.join(', ')}`);} if(audio.repeat_count!==undefined&&(!Number.isInteger(Number(audio.repeat_count))||Number(audio.repeat_count)<1)){errors.push(`${zone}.audio.repeat_count must be a positive integer`);}}} const questionAudio=item.question_audio; if(questionAudio!==undefined){if(!questionAudio||typeof questionAudio!=='object'||Array.isArray(questionAudio)){errors.push(`${zone}.question_audio must be an object`);} else {if(!has(questionAudio,'text')){errors.push(`${zone}.question_audio.text is required when question_audio is provided`);} if(questionAudio.type&&![...QUESTION_AUDIO_TYPES,'instruction'].includes(questionAudio.type)){errors.push(`${zone}.question_audio.type must be question or instruction`);} if(questionAudio.label!==undefined&&typeof questionAudio.label!=='string'){errors.push(`${zone}.question_audio.label must be a string`);} if(questionAudio.repeat_count!==undefined&&(!Number.isInteger(Number(questionAudio.repeat_count))||Number(questionAudio.repeat_count)<1)){errors.push(`${zone}.question_audio.repeat_count must be a positive integer`);}}} ['audio_text','audio_label','speakable_text','student_prompt'].forEach((k)=>{if(item[k]!==undefined&&typeof item[k]!=='string'){errors.push(`${zone}.${k} must be a string`);}}); QUESTION_AUDIO_ALIAS_FIELDS.forEach((k)=>{if(item[k]!==undefined&&typeof item[k]!=='string'){errors.push(`${zone}.${k} must be a string`);}}); if(item.repeat_after_me!==undefined&&typeof item.repeat_after_me!=='boolean'&&typeof item.repeat_after_me!=='string'){errors.push(`${zone}.repeat_after_me must be a boolean or prompt string`);} ['recordable','playback_enabled'].forEach((k)=>{if(item[k]!==undefined&&typeof item[k]!=='boolean'){errors.push(`${zone}.${k} must be a boolean`);}}); if(item.pronunciation_check!==undefined&&!PRONUNCIATION_CHECK_VALUES.includes(item.pronunciation_check)){errors.push(`${zone}.pronunciation_check must be one of ${PRONUNCIATION_CHECK_VALUES.join(', ')}`);} }
  ...
  function validateSkillPackage(skillPackage,options={}){const errors=[]; const warnings=[]; if(!skillPackage||typeof skillPackage!=='object'){return {valid:false,errors:['package must be an object'],warnings};}
    ...
    validateAudioSupport(skillPackage.lesson,'lesson',errors,warnings);
  ...
  return {QUESTION_TYPES,VISUAL_MODELS,REQUIRED_MINIMUMS,LEVEL_BANK_STATUS_VALUES,AUDIO_TYPES,QUESTION_AUDIO_TYPES,QUESTION_AUDIO_ALIAS_FIELDS,PRONUNCIATION_CHECK_VALUES,validateSkillPackage,validateLevelBanks,validateQuestionTypes,ensureQuestionId};
});
