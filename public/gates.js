(() => {
  ...
  function shell(title, bodyHtml) {
    app.innerHTML = `<main class="gates-shell"><h1>The Gates</h1><h2>${title}</h2><p class="disclaimer">${DISCLAIMER}</p>${bodyHtml}</main>`;
  }
  ...
  function renderLanding() {
    console.info(JSON.stringify({ event: 'gates_pilot_entry_started', ts: new Date().toISOString() }));
    const primaryHref = state.session?.authenticated ? '/gates/children' : '/gates/signup';
    shell('Pilot experience', `
      <p>Parents can now run the Youth Rite of Passage pilot flow end-to-end for their child.</p>
      <h3>10 Gates Overview</h3>
      <ol>${gateList().map((x) => `<li>${x}</li>`).join('')}</ol>
      <div><a class="btn" href="${primaryHref}">Start Youth Rite of Passage Assessment</a></div>
      <div><a class="btn secondary" href="/gates/signup">Parent Sign In</a></div>
    `);
  }

  function renderSignup(authRequired = false) {
    console.info(JSON.stringify({ event: 'gates_pilot_signup_viewed', ts: new Date().toISOString() }));
    shell('Parent account access', `
      ${authRequired ? '<p class="status">Please sign in to continue.</p>' : ''}
      <form id="signup-form"><h3>Create account</h3><input name="parent_name" placeholder="Parent name" required/><input name="email" type="email" placeholder="Email" required/><input name="password" type="password" placeholder="Password" required minlength="8"/><button type="submit">Create Parent Account</button></form>
      <form id="signin-form"><h3>Sign in</h3><input name="email" type="email" placeholder="Email" required/><input name="password" type="password" placeholder="Password" required/><button type="submit">Parent Sign In</button></form>
  ...
  async function renderChildren() {
    ...
    shell('Child profiles', `
      <form id="child-form"><input name="child_name" placeholder="Child name" required/><input name="child_age_band" placeholder="Age band" required/><input name="child_grade_band" placeholder="Grade band" required/><button type="submit">Create Child Profile</button></form>
      <section>${state.children.map((c) => `<article><h3>${c.child_name}</h3><p>${c.child_age_band} · ${c.child_grade_band}</p><button data-child-id="${c.child_id}">Select Child</button></article>`).join('') || '<p>No child profiles yet.</p>'}</section>
  ...
  async function renderAssessment() {
    if (!state.session?.authenticated) return renderSignup(true);
    console.info(JSON.stringify({ event: 'gates_pilot_assessment_started', ts: new Date().toISOString() }));
    ...
    shell('Youth Rite of Passage Assessment', `<p>${questions.instructions}</p><p>Select child first if needed: <a href="/gates/children">Child Profiles</a></p><form id="assessment-form">${questions.questions.map((q) => `<fieldset><legend>${q.prompt}</legend>${[1,2,3,4,5].map((v) => `<label><input type="radio" name="${q.question_id}" value="${v}" required/> ${v}</label>`).join(' ')}</fieldset>`).join('')}<button type="submit">Submit Assessment</button></form>`);
    document.querySelector('#assessment-form').addEventListener('submit', async (e) => {
      ...
      const result = await api('/api/gates/assessment/submit', { method: 'POST', body: JSON.stringify({ child_id: selected, assessment_version: questions.assessment_version, answers }) });
  ...
  async function renderResults(assessmentId) {
    if (!state.session?.authenticated) return renderSignup(true);
    console.info(JSON.stringify({ event: 'gates_pilot_results_viewed', ts: new Date().toISOString(), assessment_id: assessmentId }));
