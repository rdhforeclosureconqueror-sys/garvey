(() => {
  ...
  async function renderGateDetail(childId, gateNumber) {
    ...
    shell(`Gate ${gate.gate_number}: ${gate.name}`, `<p><strong>Current stage:</strong> ${detail.stage}</p><h3>Core lesson</h3><p>${gate.core_lesson || ''}</p><h3>Child learning statement</h3><p>${gate.child_learning_statement || ''}</p><h3>Reflection questions</h3><ul>${(gate.reflection_questions || []).map((x) => `<li>${x}</li>`).join('')}</ul><h3>Journal prompts</h3><ul>${(gate.journal_prompts || []).map((x) => `<li>${x}</li>`).join('')}</ul><h3>Developing signs</h3><ul>${(gate.developing_signs || []).map((x) => `<li>${x}</li>`).join('')}</ul><h3>Integration signs</h3><ul>${(gate.integration_signs || []).map((x) => `<li>${x}</li>`).join('')}</ul><h3>Ceremony</h3><p>${gate.ceremony || ''}</p><h3>Practice progress</h3><p>${p.progress_percent}% (${p.status})</p><p><button class="btn" id="progress-plus">+10% Practice</button> <a class="btn secondary" href="/gates/child/${childId}/gates">Back to Gates Map</a></p>`);
