...
const ArchetypeCards = {
      ...
      <div class="archetype-top3-grid">
        ${cards.map((card) => `
          <button type="button" class="archetype-card-btn" data-archetype-id="${card.id}">
            <div class="archetype-card-media">${card.svg || ""}</div>
  ...
  async openCard(id) {
    ...
    modal.innerHTML = `
      <div class="archetype-modal" role="dialog" aria-modal="true" aria-label="${card.name}">
        ...
        <div class="archetype-modal-media">${card.svg || ""}</div>
  ...
  _injectStyles() {
    ...
    style.textContent = `
      ...
      .archetype-card-media svg{width:100%;height:auto;display:block;border-radius:8px}
      ...
      .archetype-modal-media svg{width:100%;display:block;border-radius:10px;margin-bottom:10px}
