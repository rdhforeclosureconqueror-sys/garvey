...
test("assessment submit auth/ownership/persistence and latest profile", async () => {
  const { server, baseUrl, pool } = await startServer();
  try {
    const unauth = await fetch(`${baseUrl}/api/gates/assessment/submit`, { method:"POST", headers:{"content-type":"application/json"}, body: JSON.stringify({}) });
    ...
    const malformed = await fetch(`${baseUrl}/api/gates/assessment/submit`, { method:"POST", headers:{"content-type":"application/json", cookie:cookieA}, body: JSON.stringify({ child_id:childId, assessment_version:"bad", answers:"x" }) });
    assert.equal(malformed.status, 400);
    const forbidden = await fetch(`${baseUrl}/api/gates/assessment/submit`, { method:"POST", headers:{"content-type":"application/json", cookie:cookieB}, body: JSON.stringify({ child_id:childId, assessment_version:"gates_parent_observation_v1", answers:validAnswers }) });
    assert.equal(forbidden.status, 403);

    const ok = await fetch(`${baseUrl}/api/gates/assessment/submit`, { method:"POST", headers:{"content-type":"application/json", cookie:cookieA}, body: JSON.stringify({ child_id:childId, assessment_version:"gates_parent_observation_v1", answers:validAnswers }) });
    assert.equal(ok.status, 201);
    const payload = await ok.json();
    assert.equal(payload.ok, true);
    assert.match(payload.assessment_id, /^ga_/);
    assert.equal(pool.state.assessments.length, 1);
    assert.equal(pool.state.progress.length, 10);

    const profileRes = await fetch(`${baseUrl}/api/gates/children/${childId}/profile`, { headers:{ cookie:cookieA } });
    assert.equal(profileRes.status, 200);
    const profile = await profileRes.json();
    assert.ok(profile.latest_gates_profile);
    assert.equal(Array.isArray(profile.latest_gate_map), true);
