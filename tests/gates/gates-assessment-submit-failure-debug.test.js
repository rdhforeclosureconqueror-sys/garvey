...
const { QUESTIONS, GATES_ASSESSMENT_VERSION } = require('../../gates/gatesAssessmentQuestions');

function createMockPool() {
  let tenantId = 1; let userId = 1; let parentId = 1; let childId = 1; let assessmentId = 1;
  const state = { tenants:[{id:tenantId++,slug:'the-gates',name:'The Gates'}], users:[], memberships:[], sessions:[], parents:[], children:[], assessments:[], progress:[] };
...
const browserAnswers = QUESTIONS.map((q) => ({ question_id: q.question_id, value: 'often' }));

test('assessment submit failure diagnostics and success payload path', async () => {
  const { server, baseUrl, pool } = await startServer();
  try {
    assert.equal(browserAnswers.every((a) => typeof a.value === 'string' && ['rarely','sometimes','often','consistently'].includes(a.value)), true);
    ...
    const create = await fetch(`${baseUrl}/api/gates/children`, { method:'POST', headers:{'content-type':'application/json', cookie:cookieA}, body:JSON.stringify({ child_name:'Luna', child_age_band:'4-6', child_grade_band:'Pre-K', metadata:{ source:'test' } }) });
    const childId = (await create.json()).child_profile.child_id;

    const missingChild = await fetch(`${baseUrl}/api/gates/assessment/submit`, { method:'POST', headers:{'content-type':'application/json', cookie:cookieA}, body: JSON.stringify({ assessment_version:GATES_ASSESSMENT_VERSION, answers:browserAnswers }) });
    assert.equal(missingChild.status, 400);
    assert.equal((await missingChild.json()).error, 'No child selected');

    const invalidValue = await fetch(`${baseUrl}/api/gates/assessment/submit`, { method:'POST', headers:{'content-type':'application/json', cookie:cookieA}, body: JSON.stringify({ child_id: childId, assessment_version:GATES_ASSESSMENT_VERSION, answers:[{ question_id: QUESTIONS[0].question_id, value: '5' }] }) });
    assert.equal(invalidValue.status, 400);
    assert.equal((await invalidValue.json()).error, 'Invalid answer value');

    const forbidden = await fetch(`${baseUrl}/api/gates/assessment/submit`, { method:'POST', headers:{'content-type':'application/json', cookie:cookieB}, body: JSON.stringify({ child_id: childId, assessment_version:GATES_ASSESSMENT_VERSION, answers:browserAnswers }) });
    assert.equal(forbidden.status, 403);

    const ok = await fetch(`${baseUrl}/api/gates/assessment/submit`, { method:'POST', headers:{'content-type':'application/json', cookie:cookieA}, body: JSON.stringify({ child_id: childId, assessment_version:GATES_ASSESSMENT_VERSION, answers:browserAnswers }) });
    assert.equal(ok.status, 201);
    const payload = await ok.json();
    assert.equal(payload.ok, true);
    assert.match(payload.assessment_id, /^ga_/);
    assert.equal(typeof payload.next_route, 'string');
