...
function mkPool(){let ids={t:1,u:1,p:1,c:1,a:1}; const st={tenants:[{id:1,slug:'the-gates'}],users:[],memberships:[],sessions:[],parents:[],children:[],assessments:[],progress:[]};
 ...
 if(q.includes('SELECT id, created_at FROM gates_assessments WHERE parent_id')){const rows=st.assessments.filter(a=>a.parent_id===params[0]&&String(a.child_id)===String(params[1])).sort((a,b)=>new Date(b.created_at)-new Date(a.created_at)); return{rows:rows.slice(0,1).map(r=>({id:r.id,created_at:r.created_at}))};}
