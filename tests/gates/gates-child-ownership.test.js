...
function createMockPool() {
  ...
  const api = { state, connect: async()=>({query: async(sql,p=[])=>api.query(sql,p), release(){}}), query: async(sql, params=[])=>{
    ...
    if (q.includes("FROM gates_child_profiles c WHERE c.parent_id = $1") || q.includes("SELECT id, first_name FROM gates_child_profiles WHERE parent_id = $1")) return {rows:state.children.filter(c=>c.parent_id===Number(params[0])).map(c=>({id:c.id,first_name:c.first_name}))};
