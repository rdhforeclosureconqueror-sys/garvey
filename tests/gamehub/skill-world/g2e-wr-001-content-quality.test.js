...
const packagePath = path.join(root, 'public/gamehub/skill-world/content/G2E_WR_001.skill-package.v1.json');
...
test('package exists with five complete banks and 50 unique question IDs', () => {
  ...
  assert.deepEqual(pkg.level_banks.map(({ level_id }) => level_id), [
    'G2E_WR_001_LVL1', 'G2E_WR_001_LVL2', 'G2E_WR_001_LVL3',
    'G2E_WR_001_LVL4', 'G2E_WR_001_MIXED',
  ]);
...
test('authored answer positions are balanced from 15 to 35 percent', () => {
  ...
  assert.deepEqual(positions, [8, 10, 7, 10]);
