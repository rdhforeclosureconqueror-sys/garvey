...
const packagePath = path.join(root, 'public/gamehub/skill-world/content/G2E_WR_003.skill-package.v1.json');
...
test('package exists with five complete banks and 50 unique question IDs', () => {
  ...
  assert.deepEqual(pkg.level_banks.map(({ level_id }) => level_id), [
    'G2E_WR_003_LVL1', 'G2E_WR_003_LVL2', 'G2E_WR_003_LVL3',
    'G2E_WR_003_LVL4', 'G2E_WR_003_MIXED',
  ]);
...
test('every question has complete instructional content and a unique three-step hint ladder', () => {
...
test('displayed choices are normalized-unique with exactly one correct answer', () => {
...
test('all authored interaction types have renderer and evaluation support', () => {
  const supportedTypes = new Set(['multiple_choice', 'short_response', 'sequencing', 'writing_response']);
