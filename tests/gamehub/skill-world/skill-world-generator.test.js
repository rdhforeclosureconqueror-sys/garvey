...
assert.match(skillWorldIndex,/function speakAudio\(button\)\{hideAudioUnavailable\(button\);/,'click handler clears old fallback state before attempting audio playback');
...
assert.match(skillWorldIndex,/playSavedAudio\(button\)\.then\(\(played\)=>\{if\(!played\)speakWithBrowserSpeech\(button\);\}\)/,'audio_url playback path is attempted before browser speech fallback');
