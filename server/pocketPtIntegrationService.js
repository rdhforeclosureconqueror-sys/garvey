...
const VERSION = 1;
const EVENT_STATUSES = new Set(["AVAILABLE", "IN_PROGRESS", "COMPLETED", "COACH_STOPPED", "EXCUSED", "ERROR"]);
const ASSIGNMENT_STATUS = { AVAILABLE:"AVAILABLE", IN_PROGRESS:"IN_PROGRESS", COMPLETED:"COMPLETED", COACH_STOPPED:"COACH_STOPPED", EXCUSED:"EXCUSED", ERROR:"ERROR" };
...
function validateEvent(body) {
  ...
  if(Number(body.integration_version ?? body.version)!==VERSION) throw integrationError(400,"pocketpt_event_version_unsupported","The Pocket PT event version is unsupported.");
  ...
  if(!["fitness_assignment.available","fitness_assignment.in_progress","fitness_assignment.completed","fitness_assignment.coach_stopped","fitness_assignment.excused","fitness_assignment.error"].includes(body.event_type)) throw integrationError(400,"pocketpt_event_invalid","The Pocket PT event type is invalid.");
  if(!EVENT_STATUSES.has(body.status)) throw integrationError(400,"pocketpt_status_invalid","The Pocket PT status is invalid.");
  if(body.status==="COMPLETED" && (!body.completed_at || Number.isNaN(Date.parse(body.completed_at)))) throw integrationError(400,"pocketpt_event_invalid","A valid completion time is required.");
  return body;
}
...
async function launch(pool, req) {
  ...
  const payload={iss:"GARVEY",aud:"POCKET_PT",integration_version:VERSION,assignment_ref:assignment.assignment_ref,subject_ref:subject.subject_ref,source_application:"leader_within",requirement_type:"POCKET_PT_SESSION",leader_within_context:{week:Number(row.week_number),session:String(row.session_code).toUpperCase()},return_url:String(process.env.POCKETPT_RETURN_URL||"/the-leader-within/my-program"),iat:now,exp:now+300,jti};
...
async function receiveEvent(pool, req) {
  ...
  try {
    ...
    const inserted=(await client.query(`INSERT INTO leader_within_integration_events (event_id,event_type,provider,assignment_ref,status,integration_version,occurred_at) VALUES ($1,$2,$3,$4,$5,$6,$7) ON CONFLICT (provider,event_id) DO NOTHING RETURNING id`,[event.event_id,event.event_type,PROVIDER,event.assignment_ref,event.status,VERSION,event.completed_at||event.occurred_at||null])).rows[0];
    ...
    await client.query(`UPDATE leader_within_external_fitness_assignments SET status=$1,provider_assignment_ref=COALESCE($2,provider_assignment_ref),provider_program_ref=COALESCE($3,provider_program_ref),provider_session_ref=COALESCE($4,provider_session_ref),started_at=CASE WHEN $1='IN_PROGRESS' THEN COALESCE(started_at,NOW()) ELSE started_at END,completed_at=CASE WHEN $1='COMPLETED' THEN COALESCE(completed_at,$5::timestamptz) ELSE completed_at END,completion_source=CASE WHEN $1='COMPLETED' THEN 'POCKET_PT' ELSE completion_source END,last_provider_sync_at=NOW(),provider_event_id=$6,updated_at=NOW() WHERE id=$7`,[ASSIGNMENT_STATUS[event.status],event.provider_assignment_ref||null,event.program_ref||null,event.session_ref||null,completedAt,event.event_id,assignment.id]);
...
module.exports={VERSION,PROVIDER,validateEvent,signLaunchToken,expectedEventSignature,authenticateEvent,launch,receiveEvent};
