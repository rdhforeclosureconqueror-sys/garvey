...
async function getYouthDashboard(pool, req) {
  ...
  const reflections = (await pool.query(`SELECT reflection_type,submitted_at FROM leader_within_reflections WHERE enrollment_id=$1 AND week_number=$2 ORDER BY submitted_at DESC`, [row.id, row.current_week])).rows;
  ...
  const w = curriculumWeek(row.current_week); const phase = phaseForWeek(row.current_week); const featured = row.alternate_story_title || STORY_MAP[row.current_session] || w.stories?.[0]?.title || STORY_MAP.A;
  const session = buildYouthSession(row.current_session, w, featured, practice, progress);
  const progressCounts = { stories_completed: progress.filter((p) => p.status === "completed").length, leadership_practices: practice ? 1 : 0, reflections: reflections.length, movement_missions: 0, attendance: progress.length, assessment_completion: snap ? 1 : 0 };
  const completionItems = [progressCounts.stories_completed > 0, !!practice, reflections.length > 0, progressCounts.movement_missions > 0, !!snap];
  const completionPercentage = Math.round((completionItems.filter(Boolean).length / completionItems.length) * 100);
  return { csrf_token: csrfTokenForRequest(req), program: { title: row.program_title, duration_weeks: row.duration_weeks }, participant: { first_name: (row.participant_name || "Friend").split(/[ @]/)[0] }, current: { phase: `Phase ${phase.id}: ${phase.name}`, week: row.current_week, session: row.current_session, session_title: sessionTitle(row.current_session), mission: currentMission(row.current_session, !!practice) }, todays_mission: { learn: session.featured_story?.title || w.title, practice: practice?.practice_id || "Choose today's leadership practice", move: movementMissionForWeek(w).assignment, reflect: (w.afterCore || w.after || ["What happened?"])[0], status: practice && reflections.length ? "In Progress" : "Ready" }, weekly_story_library: session.weekly_story_library, assessment: assessmentCard(snap), archetype_introduction: archetypeIntroduction(), leadership_profile: leadershipProfile(snap), practice_options: practiceOptionsForWeek(w), practice, reflection_workflow: { prompts: ["What happened?", "How did it go?", "What was difficult?", "What did you learn?", "Would you do anything differently?"], response_modes: ["text", "voice", "simple_response"], submissions_visible_to_facilitator: true }, movement_mission: movementMissionForWeek(w), progress: progressCounts, weekly_progress: { ...progressCounts, completion_percentage: completionPercentage }, my_growth: { leadership_growth: snap ? "Profile started" : "Assessment pending", consistency: `${progress.length} session check-ins`, reflection_streak: reflections.length, participation: progress.length, facilitator_recognitions: 0, badges: completionPercentage >= 80 ? ["Weekly Flow Builder"] : [], milestones: snap ? ["Leadership assessment completed"] : [] }, session };
}
