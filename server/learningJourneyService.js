...
async function buildLearningJourney(pool, { childId, parentProfileId = null, childName = "" } = {}) {
  ...
  const assessments = await queryOptional(pool, `SELECT s.session_id, s.learner_id::text AS child_id, s.parent_profile_id::text, s.assessment_role, s.grade, s.subject, s.status, s.current_question_position, s.created_at, s.updated_at, s.completed_at,
    COUNT(i.id)::int AS maximum_score,
    COUNT(r.id) FILTER (WHERE r.score_status = 'correct')::int AS raw_score,
    COUNT(r.id) FILTER (WHERE r.score_status IN ('correct', 'incorrect'))::int AS answered_count,
    (COUNT(i.id) - COUNT(r.id) FILTER (WHERE r.score_status IS DISTINCT FROM 'omitted'))::int AS skipped_count,
    CASE WHEN COUNT(i.id) > 0 THEN ROUND(100.0 * COUNT(r.id) FILTER (WHERE r.score_status = 'correct') / COUNT(i.id))::int ELSE NULL END AS score_percent
    FROM assessment_sessions s
    LEFT JOIN assessment_session_items i ON i.session_id = s.id
    LEFT JOIN assessment_responses r ON r.session_item_id = i.id
    WHERE s.learner_id::text=$1${parentFilter}
    GROUP BY s.id
    ORDER BY COALESCE(s.completed_at, s.updated_at, s.created_at) DESC`, params);
  ...
  const latestCompletedAssessment = completedAssessments[0] || null;
  const latestScore = latestCompletedAssessment?.score_percent ?? scoreFrom(latestProgress);
  ...
  return {
    ...
    history: { completed_lessons: completedLessons, skill_worlds: skillWorlds, completed_skill_worlds: skillWorlds.filter((r) => r.status === "completed"), completed_checkpoints: checkpointRows, assessment_history: assessments, scores: { latest_score: latestScore, latest_raw_score: latestCompletedAssessment?.raw_score ?? null, latest_maximum_score: latestCompletedAssessment?.maximum_score ?? null, skill_world_scores: skillWorlds.map((s) => ({ skill_id: s.skill_id, score_percent: s.score_percent })) }, attempts: { assessment_attempts: assessments.length, checkpoint_attempts: toNumber(latestProgress?.checkpoint_attempts), skill_world_attempts: skillWorlds.reduce((sum, s) => sum + s.attempts, 0) }, completion_timestamps: { latest_assessment_completed_at: toIso(latestCompletedAssessment?.completed_at), last_activity_at: latestDate([...assessments, ...skillWorldsRaw, ...adaptiveRows, ...checkpointRows], ["completed_at", "updated_at", "created_at"]) }, mastery_changes: checkpointRows.map((r) => ({ skill_id: r.skill_id, checkpoint_id: r.checkpoint_id, mastery_band: r.mastery_band_after, occurred_at: toIso(r.created_at) })) },
