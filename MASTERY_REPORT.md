...
| `public/intake.html` | business intake renderer | GET `/api/questions?assessment=business_owner`; POST `/api/intake` | `tenant` | `questions/answers/currentIndex` in-memory | inline `showError` + alert email |
...
### Answer object shape used by FE submissions and backend validation
```json
{ "qid": "BO1|CU1", "answer": "A|B|C|D" }
```

### Submission contracts
- Business intake: `POST /api/intake` with `{email, tenant, answers:[...]}`
  -> `{success:true, assessment_type:"business_owner", ...}`
- Customer VOC: `POST /voc-intake` with `{email, tenant, answers:[...]}`
  -> `{success:true, assessment_type:"customer", ...}`

### Mismatch report summary (fixed)
- **DOM mismatch** on intake page caused render failure (`Missing DOM elements`) because script expected IDs not present.
- **Placeholder question/option text** existed from seed source (`Customer Question X`, `Option A/B/C/D`) and propagated to UI.
- **Broken assessment CTA** on tenant landing created dead UX path; removed from homepage and kept admin as canonical business intake entry.
