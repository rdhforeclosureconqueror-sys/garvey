...
| Opinion writing | `G2E_WR_001` | Not built | **Planned / Needs writing validation** | Existing `sentence_builder` and `writing_checklist` help; planned `opinion_reason_chart`, `paragraph_builder`, and `rubric_scored_writing` require stronger deterministic writing validation. | Do not build as exact-answer writing only. Add rubric dimensions for opinion, reason, and closing. |
| Informative writing | `G2E_WR_002` | Not built | **Planned / Needs writing validation** | Plan requires fact cards, topic/detail chart, fact selection, linking words, and paragraph builder. | Build after writing validation/fact-bank metadata exists. |
| Narrative writing | `G2E_WR_003` | Not built | **Planned / Needs writing validation** | Existing sequence renderers help but narrative builder and rubric validation are not production-proven. | Build after opinion/informative writing validation pattern is proven. |
